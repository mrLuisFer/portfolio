...
import { noSelect } from '../../styles/no-select'
...
const AboutResumeWave = styled.img`
  display: inline-block;
  position: absolute;
  top: 0;
  left: 0;
...
const AboutResumeInfoTitleContainer = styled.div`
...
const AboutResumeInfoTitle = styled.h2`
...
const AboutResumeInfoTitleText = styled.p`
  color: var(--beau-blue);
  opacity: 0.8;
...
const AboutResumeInfoText = styled.div`
  p {
    color: #ffffff;
    font-weight: 400;
    cursor: text;
  ...
  a {
    ...
    &:hover {
      ...
      @media (min-width: $large) {
        @include after-btn('mrLuisFer | Curriculum Vitae', 3.5rem);
      }
    }
  }
`
...
const AboutResumeInfoIllustration = styled.img`
  width: 300px;
  transition: var(--transition);
  display: flex;
  margin: 0 auto;
